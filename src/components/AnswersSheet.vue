  ...
  <Sheet>
    ...
    <SheetContent class="flex flex-col justify-between">
      ...
      <section class="my-4 h-full space-y-2 overflow-auto">
        <div v-for="answer in answers" class="border border-slate-300 p-4 rounded-2xl">
      ...
      <SheetFooter>
        <section v-if="toggleShow" class="my-4">
          <textarea class="w-full h-32 p-4 mt-4 text-sm text-gray-600 placeholder-gray-400 bg-gray-100 border border-gray-300 rounded-lg" placeholder="Escribe tu respuesta"></textarea>
          <button class="w-full bg-indigo-600 text-white p-4 rounded-2xl" >
            Enviar respuesta
          </button>
        </section>

          <button v-if="!toggleShow" @click="addAnswer"
